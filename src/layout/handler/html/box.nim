...
proc newBoxBorder*(color: CSSColor, width: uint): BoxBorder =
...
proc newBox*(boxType: BoxType, boxFlags: BoxFlags, dom: DOM, id: string, 
             ...
             boxBorder: BoxBorder, width: tuple[max, min: int], text: string, space: int, 
  ...
  Box(boxType: boxType, boxFlags: boxFlags, dom: dom, id: id, next: next, 
      ...
      dimensions: dimensions, descendants: descendants, margin: margin, 
      padding: padding, boxBorder: boxBorder, width: width, text: text, 
      space: space, columns: columns, rows: rows, startColumn: startColumn, column: column, 
