...
proc evaluateRGBAFunction*(value: CSSValue): Option[ColorRGBA] =
  ...
  failCond value.fn.arguments.len == 4
    # We need the r, g, b and a components. If not found, return nothing.
...
proc evaluateRGBFunction*(value: CSSValue): Option[ColorRGB] =
  ...
  ## https://www.w3.org/TR/css-color-4#rgb-functions
  failCond value.fn.arguments.len == 3
    # We need the r, g and b components. If not found, return nothing.
...
proc evaluateRGBXFunction*(value: CSSValue): Option[ColorRGBA] =
  ...
  assert(
    value.kind == cssFunction,
    "evaluateRGBXFunction() called on CSS value which isn't a functional notation",
  )
